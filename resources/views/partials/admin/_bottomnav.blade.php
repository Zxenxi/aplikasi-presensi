        ...
        <div class="flex justify-around items-center h-full max-w-md mx-auto px-2" id="bottom-navigation">
            @auth
                @if (Auth::user()->isSuperAdmin() || Auth::user()->isPetugasPiket())
                    ...
                    {{-- Dashboard (terlihat untuk Admin dan Petugas Piket) --}}
                    ...
                    @if (Auth::user()->isSuperAdmin())
                        <a href="{{ route('admin.users.index') }}" title="Pengguna"
                            ...
                            <i data-lucide="users" class="h-5 w-5 mb-1"></i><span
                                class="text-xs font-medium">Pengguna</span>
                        </a>
                    @endif

                    {{-- Kelas (HANYA untuk Super Admin) --}}
                    @if (Auth::user()->isSuperAdmin())
                        <a href="{{ route('admin.classes.index') }}" title="Kelas"
                            ...
                            <i data-lucide="building" class="h-5 w-5 mb-1"></i><span
                                class="text-xs font-medium">Kelas</span>
                        </a>
                    @endif
                    ...
                    {{-- Laporan (terlihat untuk Admin dan Petugas Piket) --}}
                    ...
                    {{-- Pengaturan (HANYA untuk Super Admin) - tidak muat di bottom nav, biasanya di menu user --}}
                    @if (Auth::user()->isSuperAdmin())
                        {{-- <a href="{{ route('admin.settings.edit') }}" title="Pengaturan Aplikasi" class="bottom-nav-item flex flex-col items-center justify-center w-1/5 pt-1 {{ request()->routeIs('admin.settings*') ? 'active' : '' }}">
                            <i data-lucide="settings" class="h-5 w-5 mb-1"></i><span class="text-xs font-medium">Pengaturan</span>
                        </a> --}}
                    @endif
